...
from mode_choice.variables.pt_java import pt_variables
from mode_choice.variables.car import car_variables

# costs
from mode_choice.cost.car import car_cost
from mode_choice.cost.pt_java import pt_cost
...
def execute(context):
    ...
    car_vars = car_variables(context, car_trips)
    ...
    num_nans = car_trips.isna().sum().sum()
    logger.info(f"\t \t There are {num_nans} nans in car trips after computing car variables.")
    ...
    num_nans = car_trips["car_cost_CHF"].isna().sum()
    logger.info(f"\t \t There are {num_nans} nans in car trips after computing car cost.")
    
    # 2. pt cost
    cost_pt = pt_trips[["person_id", "trip_id", "pt_distance_km"]].rename(columns={"pt_distance_km": "distance_km"}).copy()
    ...
    num_nans = pt_trips.isna().sum().sum()
    logger.info(f"\t \t There are {num_nans} nans in pt trips after computing pt cost.")
