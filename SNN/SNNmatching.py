...
def parallel_statistical_matching(context, df_source, source_identifier, weight, df_target, target_identifier, columns,
    ...
    processes = 1# context.config("threads")
...
def SNNmatching_match(context, df_persons, timeuse_participants, timeuseplus_days, agents_wfh):

    df_persons = df_persons[df_persons["person_id"].isin(agents_wfh)]

    df_source = timeuse_participants.copy()
    df_target = df_persons.copy()

    columns = ["car_availability", "age_class", "sex", "marital_status"]

    df_target.loc[df_target["age_class"]==2, "age_class"] = 3
    df_source.loc[:, "weight"] = 1.0
    df_source.loc[df_source["car_availability"].isna(), "car_availability"] = 0

    df_assignment, levels = parallel_statistical_matching(
        context,
        df_source, "participant_id", "weight",
        df_target, "person_id",
        columns,
    ...
    df_matching = df_target[["person_id", "participant_id"]]

    timeuseplus_days = timeuseplus_days.drop_duplicates().sort_values(by = "participant_id")

    timeuseplus_days = timeuseplus_days[["participant_id", "person_day_id"]].groupby(["participant_id"])['person_day_id'].apply(list).reset_index()
    timeuseplus_days.columns = ["participant_id", "person_day_id"]

    df_matching = df_matching.merge(timeuseplus_days, left_on = "participant_id", right_on="participant_id", how = "left")      
    df_matching.loc[:, "person_day_id"] = [random.choice(days) for days in df_matching["person_day_id"]]
