...
def frequent_weighted_sequences(data):
    ...
    total = sum(map(lambda x : x[1], pairs))
...
def get_individual_car_class(data):
...
def get_subscription_proportions(data):
    weighted_proportions = {}
    total_weight = data['person_weight'].sum()

    subscriptions = ['subscriptions_verbund',
...
def get_histogram_time(data, feature, bins=None):
    times = data[feature]
    bins = np.arange(0, 86401, 1800)  # From 0 to 86400, step = 1800 seconds (30 min)
    # NOTE: left or right inclusive/exclusive bin
