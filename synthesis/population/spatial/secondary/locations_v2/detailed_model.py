...
from .NNModel import MNLModel, train_with_mask, DetailedLevel2Wrapper
...
logger = logging.getLogger("synpp: detailed_model")

MODEL_NAME = "detailed_model.pt"
...
def configure(context):
	...
	context.config("overwrite_detailed_model_if_exists", True)
	context.config("detailed_model_batch_size", 256)
	context.config("detailed_model_epochs", 40)
	context.config("detailed_model_torch_num_threads", 16)
	context.config("random_seed")


def execute(context):
	logger.info("Training detailed model for level2-within-level1 secondary location choice...")
	...
	missing_h3_cols = [c for c in required_h3_cols if c not in h3_geo_level2.columns]
	if missing_h3_cols:
	...
	df = df[df["purpose"].isin(SECONDARY_ACTIVITIES)].dropna(
		subset=["destination_level_0", "destination_level_1", "destination_level_2"]
	...
	max_children = max(
		...
		for l0, l1 in df[["destination_level_0", "destination_level_1"]].itertuples(index=False)
	)
	n_samples = len(df)
	...
	with context.progress(total=n_samples, label="Detailed model: building level2 choice sets") as progress:
		for i, row in df.iterrows():
			key = (row["destination_level_0"], row["destination_level_1"])
			chosen_l2 = row["destination_level_2"]
			children = children_by_level1[key]

			valid_mask[i, :len(children)] = True
			for j, child in enumerate(children):
				cand_x[i, j] = float(centroid_x_by_l2[child])
				cand_y[i, j] = float(centroid_y_by_l2[child])
				cand_statent[i, j] = float(statent_count_l2.get(child, 0.0))
				cand_employees[i, j] = float(employees_count_l2.get(child, 0.0))
				cand_urban_core[i, j] = float(urban_core_count_l2.get(child, 0.0))
				cand_urban[i, j] = float(urban_count_l2.get(child, 0.0))
				cand_education[i, j] = float(education_count_l2.get(child, 0.0))
				cand_shop[i, j] = float(shop_count_l2.get(child, 0.0))
				cand_leisure[i, j] = float(leisure_count_l2.get(child, 0.0))
				cand_ovgk_share_a[i, j] = float(ovgk_share_a_by_l2.get(child, 0.0))
				cand_ovgk_share_b[i, j] = float(ovgk_share_b_by_l2.get(child, 0.0))
				cand_ovgk_share_c[i, j] = float(ovgk_share_c_by_l2.get(child, 0.0))
				cand_ovgk_share_d[i, j] = float(ovgk_share_d_by_l2.get(child, 0.0))
				cand_ovgk_share_none[i, j] = float(ovgk_share_none_by_l2.get(child, 0.0))
				cand_outside_fraction[i, j] = float(outside_fraction_by_l2.get(child, 0.0))

			y[i] = children.index(chosen_l2)
			progress.update()
	...
	logger.info("\t Computing detailed-model numerical features with Numba...")
	...
	X[:, :, len(numerical) + 3:] = np.broadcast_to(
		purpose_one_hot[:, None, :],
		(n_samples, max_children, purpose_one_hot.shape[1]),
	)
	X[~valid_mask] = 0.0

	logger.info("\t Training detailed model...")
	...
	wrapper = DetailedLevel2Wrapper(
		model=model,
		scaler=scaler,
		numerical_cols=numerical,
		features=features,
		children_by_level1=children_by_level1,
		level2_candidate_attributes_by_level1=level2_candidate_attributes_by_level1,
...
def plot_analysis(context, wrapper, X, valid_mask, df, children_by_level1, h3_geo_level2, centroid_x_by_l2, centroid_y_by_l2):
	...
	predicted_level2 = []
	for i, (l0, l1) in enumerate(df[["destination_level_0", "destination_level_1"]].itertuples(index=False)):
		children = children_by_level1[(l0, l1)]
	...
	logger.info("Detailed plot saved to %s", plot_path)
	plt.close(fig)
	...
	logger.info("Detailed distance distribution plot saved to %s", dist_plot_path)
	plt.close(fig)
