...
def configure(context):
    ...
    context.config("local_model_epochs", 40)
    context.config("local_model_learning_rate", 1e-2)
    context.config("local_model_torch_num_threads", 16)
...
def execute(context):
    ...
    train_choice_model(model=model, person_static_x=static_matrix, person_dynamic_x=dynamic_matrix, candidate_static_x=candidate_static_x, candidate_dynamic_x=candidate_dynamic_x,
        ...
        weight_decay=1e-3,
        num_threads=int(context.config("local_model_torch_num_threads")))
