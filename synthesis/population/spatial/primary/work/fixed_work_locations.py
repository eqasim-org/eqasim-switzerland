...
def execute(context):
    ...
    persons = context.stage("synthesis.population.sampled")[["person_id", "household_id", "home_zone_id", "home_x", "home_y", "employed","car_availability"]].copy()
    ...
    df = persons[~persons["person_id"].isin(remote_working)].reset_index(drop=True)

    # Remove those working from different places
    ...
    df = df[~df["person_id"].isin(work_different_location)].reset_index(drop=True)
    ...
    df_statent = correct_companies_number_of_employees(context, df_statent)
    ...
    for origin_idx, origin_zone in enumerate(zone_ids):
        ...
        if n_commuters == 0:
            ...
            continue
        zone_probs = pdf_matrices[origin_idx, :]
    ...
    no_comp = set()
    with context.progress(total=n, label="Assigning fixed work locations (OD+distance)") as prog:    
        for idx in range(n):
