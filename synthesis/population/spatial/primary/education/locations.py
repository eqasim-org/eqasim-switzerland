...
def execute(context):
    ...
    # TODO: This should not be left merge, but for now, inner merge is causing probelm in secondary location assignement
    df_persons = pd.merge(df_persons, df_commute, on="mz_person_id", how="left")
    ...
    for person_selector, location_selector, type, query_size in zip(filters_persons, filters_locations, education_types, query_sizes):
        ...
        distance_weight = np.nan_to_num(distance_weight, nan=1.0) # this line is added because of the left merge above, which can lead to NaN distances
                                                                  # in these cases we do not consider the distance
