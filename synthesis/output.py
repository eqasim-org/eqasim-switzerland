...
def execute(context):
    ...
    df_spatial = gpd.GeoDataFrame(df_activities, geometry="geometry",crs="EPSG:2056")
    ...
    df_spatial[df_spatial["purpose"] == "home"].drop_duplicates("household_id")[["household_id", "geometry"]].to_file(path, driver = "GPKG")
    clean_gpkg(path)
    ...
    df_spatial["geometry"] = [geo.LineString(od) for od in zip(df_spatial["preceding_geometry"], df_spatial["following_geometry"])]
