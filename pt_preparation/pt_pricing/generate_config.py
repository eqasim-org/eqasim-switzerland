...
def configure(context):
    ...
    context.config("generate_files", default = True)
    if not context.config("generate_files"):
        context.config("network_path")
        context.config("transit_schedule_path")
        context.config("transit_vehicles_path")

    if context.config("generate_files"):
...
def execute(context):
    ...
    if context.config("generate_files"):
        ...
        transit_vehicles_input_path = context.config("transit_vehicles_path")
        transit_vehicles_output_path = "%s/%stransit_vehicles.xml.gz" % (context.path(), context.config("output_prefix"))
        shutil.copyfile(transit_vehicles_input_path, transit_vehicles_output_path)
    ...
    if context.config("generate_files"):    
        ...
        network_input_path = context.config("network_path")
    ...
    if context.config("generate_files"):
        ...
        transit_schedule_input_path = context.config("transit_schedule_path")
