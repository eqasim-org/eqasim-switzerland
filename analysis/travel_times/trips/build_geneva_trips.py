...
def configure(context):
    ...
    context.config("num_geneva_trips", default=20_000)
    context.config("geneva_trip_distance_min_m", default=2_000)
    context.config("geneva_trip_distance_max_m", default=20_000)
...
def execute(context):
    ...
    num_trips = int(context.config("num_geneva_trips"))
    min_trip_distance_m = int(context.config("geneva_trip_distance_min_m"))
    max_trip_distance_m = int(context.config("geneva_trip_distance_max_m"))

    if num_trips <= 0:
        raise ValueError("num_geneva_trips must be > 0")
    if min_trip_distance_m < 0:
        raise ValueError("geneva_trip_distance_min_m must be >= 0")
    if max_trip_distance_m <= min_trip_distance_m:
        raise ValueError("geneva_trip_distance_max_m must be strictly greater than geneva_trip_distance_min_m")

    # geom canton geneva
    df_cantons = context.stage("data.spatial.cantons")
    geneva_rows = df_cantons.loc[
        df_cantons["canton_name_en"].astype(str).str.casefold() == "geneva",
        "geometry"
    ]
    if geneva_rows.empty:
        raise ValueError("Could not find canton 'geneva' in data.spatial.cantons")
    geneva_geom = geneva_rows.buffer(2_000).simplify(500).iloc[0]
    ...
    node_ids = geneva_nodes["node_id"].astype(str).values
    node_x = geneva_nodes["x"].values
    node_y = geneva_nodes["y"].values
    ...
    while len(sampled_origin_idx) < num_trips and attempts < max_attempts:
        attempts += 1
        origin_idx = int(rng.integers(0, len(geneva_nodes)))
    ...
    if len(sampled_origin_idx) < num_trips:
        raise ValueError(
            "Unable to generate the requested number of Geneva trips with the configured distance "
    ...
    sampled_origins = geneva_nodes.iloc[sampled_origin_idx].reset_index(drop=True)
    sampled_destinations = geneva_nodes.iloc[sampled_destination_idx].reset_index(drop=True)
    ...
    trips = gpd.GeoDataFrame({
        'identifier': [f'geneva_trip_{i+1}' for i in range(num_trips)],
    ...
    logger.info(
        "Generated %s Geneva trips between distinct nodes with crow-fly distance in [%sm, %sm]",
        num_trips,
        min_trip_distance_m,
        max_trip_distance_m,
    )
