...
def execute(context):
    ...
    num_trips = int(context.config("num_french_trips"))
    min_trip_distance_m = int(context.config("french_trip_distance_min_m"))
    max_trip_distance_m = int(context.config("french_trip_distance_max_m"))

    if num_trips <= 0:
        raise ValueError("num_french_trips must be > 0")
    if min_trip_distance_m < 0:
        raise ValueError("french_trip_distance_min_m must be >= 0")
    if max_trip_distance_m <= min_trip_distance_m:
        raise ValueError("french_trip_distance_max_m must be strictly greater than french_trip_distance_min_m")

    # geom canton france
    df_cantons = context.stage("data.spatial.cantons")
    cst = context.stage("data.external_population.constants")

    france_rows = df_cantons.loc[
        df_cantons["canton_name_en"].astype(str) == cst.canton_name,
        "geometry"
    ]
    if france_rows.empty:
        raise ValueError("Could not find canton 'france' in data.spatial.cantons")
    france_geom = france_rows.simplify(500).iloc[0]

    # nodes
    x_values = net.nodes["x"].values
    y_values = net.nodes["y"].values

    # nodes in canton France
    france_nodes_mask = vectorized.contains(france_geom, x_values, y_values)
    france_nodes = net.nodes.loc[france_nodes_mask, ["node_id", "x", "y"]].reset_index(drop=True)
    if len(france_nodes) < 2:
        raise ValueError("Need at least two network nodes in France to build OD trips")

    node_ids = france_nodes["node_id"].astype(str).values
    node_x = france_nodes["x"].values
    node_y = france_nodes["y"].values
    ...
    while len(sampled_origin_idx) < num_trips and attempts < max_attempts:
        attempts += 1
        origin_idx = int(rng.integers(0, len(france_nodes)))
    ...
    if len(sampled_origin_idx) < num_trips:
        raise ValueError(
            "Unable to generate the requested number of French trips with the configured distance "
    ...
    sampled_origins = france_nodes.iloc[sampled_origin_idx].reset_index(drop=True)
    sampled_destinations = france_nodes.iloc[sampled_destination_idx].reset_index(drop=True)
    ...
    trips = gpd.GeoDataFrame({
        'identifier': [f'french_trip_{i+1}' for i in range(num_trips)],
    ...
    logger.info(
        "Generated %s French trips between distinct nodes with crow-fly distance in [%sm, %sm]",
        num_trips,
        min_trip_distance_m,
        max_trip_distance_m,
    )
