...
class ModeShareAnalyzer:
    ...
    def load_microcensus_data(self, context):
        ...
        trips["geometry_destination"] = gpd.points_from_xy(trips.destination_x, trips.destination_y)
        df_dest = gpd.GeoDataFrame(trips[["geometry_destination"]], 
                                   geometry="geometry_destination", 
                                   crs="EPSG:2056")
        df_dest = df_dest.sjoin_nearest(df_municipalities, how="left")
        trips["destination_municipality"] = df_dest["municipality_type"]
        trips = trips.drop(columns=["geometry_destination"])
