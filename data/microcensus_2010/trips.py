...
def execute(context):
    ...
    df_mz_trips = pd.read_sas("%s/wege.sas7bdat" % data_path, encoding = "latin1")
    df_mz_stages = pd.read_sas("%s/etappen.sas7bdat" % data_path, encoding = "latin1")

    df_mz_trips = df_mz_trips[[
        "HHNR", "ZIELPNR", "WEGNR", "f51100", "f51400", "wzweck1", "wzweck2", "wmittel",
        "S_X_CH1903", "S_Y_CH1903", "Z_X_CH1903", "Z_Y_CH1903", "w_x_CH1903", "w_y_CH1903",
        "w_rdist", "f51800a", "f51700"
    ]]

    df_mz_stages = df_mz_stages[[
        "HHNR", "ZIELPNR", "WEGNR", "ETNR", "f51300"
    ]]

    df_mz_trips["Household_ID"] = df_mz_trips["HHNR"] * 10 + df_mz_trips["ZIELPNR"]
    df_mz_stages["Household_ID"] = df_mz_stages["HHNR"] * 10 + df_mz_stages["ZIELPNR"]
    ...
    df_passengers = df_mz_stages[["Household_ID", "WEGNR", "is_car_passenger"]].groupby(["Household_ID", "WEGNR"]).sum().reset_index()
    df_mz_trips = pd.merge(df_mz_trips, df_passengers, on = ["Household_ID", "WEGNR"], how = "left")
    ...
    df_cost = pd.DataFrame(df_mz_stages[["HHNR", "ZIELPNR", "WEGNR", "f51330"]], copy = True)
    df_cost.columns = ["household_id", "pers_id", "trip_id", "parking_cost"]
    df_cost["person_id"] = df_cost["household_id"] * 10 + df_cost["pers_id"]
