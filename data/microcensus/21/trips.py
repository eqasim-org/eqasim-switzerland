...
def execute(context):
    ...
    df_mz_trips   = pd.read_csv("%s/microcensus/21/wege.csv" % data_path, sep=";", encoding = "latin1")
    df_mz_stages  = pd.read_csv("%s/microcensus/21/etappen.csv" % data_path, sep=";", encoding = "latin1")
    df_mz_persons = pd.read_csv("%s/microcensus/21/zielpersonen.csv" % data_path, sep=";", encoding = "latin1", parse_dates = ["USTag"])
    df_households = pd.read_csv("%s/microcensus/21/haushalte.csv"  % data_path, sep = ";", encoding = "latin1")
    
    df_mz_trips = df_mz_trips[[
    "HHNR", "WEGNR", "f51100", "f51400", "wzweck1", "wzweck2", "wmittel1",
    "S_X", "S_Y", "Z_X", "Z_Y",
    "w_rdist"
    ]]
    ...
    df_hhl_home = df_households[["HHNR", "W_X", "W_Y"]]

    df_mz_persons_work      = df_mz_persons[["HHNR", "A_X", "A_Y", "AU_X", "AU_Y"]]
    df_mz_persons_locations = df_mz_persons_work.merge(df_hhl_home, on = "HHNR")

    df_mz_trips = df_mz_trips.merge(df_mz_persons_locations, on = "HHNR", how = "left")
    ...
    df_mz_trips["mode"] = df_mz_trips["wmittel1"].map(mode_map)

    df_mz_trips["mode_detailed"] = df_mz_trips["mode"]
    df_mz_trips.loc[df_mz_trips["wmittel1"] == 1, "mode_detailed"]  = "plane"
    df_mz_trips.loc[df_mz_trips["wmittel1"] == 10, "mode_detailed"] = "taxi"
    df_mz_trips.loc[df_mz_trips["wmittel1"] == 11, "mode_detailed"] = "UberTaxi"

    # Find passenger trips
    df_mz_stages["is_car_passenger"] = df_mz_stages["f51300"]==10
    ...
    for mz_attribute, df_attribute in [("Z", "destination"), ("S", "origin"), ("W", "home"), ("A", "work"), ("AU", "education")]:
        coords = df_mz_trips[["%s_X" % mz_attribute, "%s_Y" % mz_attribute]].values
    ...
    unknown_ids = set(df_mz_trips[
        (df_mz_trips["mode"] == "unknown") | (df_mz_trips["purpose"] == "unknown") | 
                    (df_mz_trips["origin_x"] == np.inf) | (df_mz_trips["origin_y"] == np.inf) |
                    (df_mz_trips["destination_x"] == np.inf) | (df_mz_trips["destination_y"] == np.inf)                   
    ]["person_id"])
    ...
    df_start = df_mz_trips[df_mz_trips["trip_id"]==1].copy()
    ...
    missing_origin_purpose = df_start[df_start["origin_purpose"].isna()].copy()
    logger.info("  Assigned origin_purpose of initial trip to home to %d persons" % len(start_from_home))
    logger.info("  Assigned origin_purpose of initial trip to work to %d persons" % len(start_from_work))
    logger.info("  Assigned origin_purpose of initial trip to education to %d persons" % len(start_from_education))
    logger.info("  Number of agents with missing initial purpose: %d" % len(missing_origin_purpose))

    if len(missing_origin_purpose) > 0:
        logger.info("   Investigating stages for these %d agents." % len(missing_origin_purpose))
        ...
        def clean_f52950(val):
            ...
            origin      = "-".join(clean_parts)
            values      = list(set(list(origin.split("-"))))[0]
            return values
    ...
    if len(have_consecutive_work_or_edu) > 0:
        logger.info("INFO fixing consecutive work and education activities for %d agents." % len(have_consecutive_work_or_edu))
        ...
        consecutive_actchains = activity_chains[activity_chains['has_consecutive_work_or_edu']].copy()
        ...
        for person_id in have_consecutive_work_or_edu:
            df_person    = df_mz_trips[df_mz_trips["person_id"]==person_id].sort_values(by="trip_id").copy()
    ...
    for person_id in df_home_2ndcase_ids:
        df_person  = df_mz_trips[df_mz_trips["person_id"]==person_id].copy()
    ...
    for person_id in df_home_3rdcase_ids:
        ...
        other_home_coord = df_home_coordinates[(df_home_coordinates["person_id"]==person_id) & (~df_home_coordinates["is_reported_home"])].copy()
    ...
    for person_id in df_home_4thcase_ids:
        df_person             = df_mz_trips[df_mz_trips["person_id"]==person_id].copy()
        home_coord            = df_home_coordinates[(df_home_coordinates["person_id"]==person_id) & (~df_home_coordinates["is_reported_home"])].copy()
    ...
    stages  = pd.read_csv("%s/microcensus/21/etappen.csv" % data_path, sep=";", encoding = "latin1")
    stages  = stages[["HHNR", "WEGNR", "WP", "ETNR", "DMOD", "f51300", "f52900", "f52950", "f51100time", 
                    "f51400time", "S_X", "S_Y",
                            "Z_X", "Z_Y"]]
    ...
    stages = stages[stages["person_trip_id"].isin(loops["person_trip_id"])].copy()

    stages['stage_count'] = stages.groupby('person_trip_id')['stage_id'].transform('count')
    multi_stages  = stages[stages["stage_count"]>1].copy()
    single_stages = stages[stages["stage_count"]==1].copy()

    loops_singlestage = loops[loops["person_trip_id"].isin(single_stages["person_trip_id"])].copy()
    loops_singlestage.loc[:, "mode"] = loops_singlestage["mode"] + "_loop"

    loops_multistage = loops[loops["person_trip_id"].isin(multi_stages["person_trip_id"])].copy()
    ...
    def process_useful_stages(multi_stages, loops_multistage):
        ...
        mode_map = {
                -99: "unknown",               # Pseudoetappe
                -98: "unknown",               # Keine Antwort
                -97: "unknown",               # Weiss nicht
                1: "walk",                    # Zu Fuss
                2: "bike",                    # Velo
                ...
                8: "car_passenger",    # Motorrad als Mitfahrer
                9: "car",                     # Auto als Fahrer
                10: "car_passenger",          # Auto als Mitfahrer
                11: "pt",                     # Bahn/Zug
                12: "pt",                     # Bus/Postauto/Schulbus
                13: "pt",                     # Tram/Metro
                14: "pt",                     # Taxi klassisch
                15: "pt",                     # Taxi-ähnliche Fahrdienste
                16: "pt",                     # Reisecar
                17: "truck",                  # Lastwagen
                18: "pt",                     # Schiff/Boot
                19: "pt",                     # Flugzeug/Luftfahrzeug
                20: "pt",                     # Zahnradbahn/Seilbahnen/Lifte
                21: "bike",                   # Fahrzeugähnliche Geräte
                95: "other"                   # Anderes
            }
    ...
    for col in ["origin_x", "origin_y", "destination_x", "destination_y"]:
        multi_stages.loc[:, col] = multi_stages[col].astype(int)
