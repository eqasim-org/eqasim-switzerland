...
def get_canton_trip_data(path="output_trips.csv", by="origin"):
    """
    Generalized function to output trip data by canton, treating each canton as either origin or destination.
    Args:
        path: Path to the CSV file.
        by: Either 'origin' or 'destination'. Determines which canton role to use for grouping/output.
    """
    df = pd.read_csv(path, sep=';')
    ...
    # Restructure the data to create nested time_bins
    nested_data = {}
    ...
    output_dir = f"plot_data_{by}"
    os.makedirs(output_dir, exist_ok=True)

    canton_list = df[by].unique()
    for canton in canton_list:
        # Filter records for this canton (as origin or destination)
        idx = 0 if by == "origin" else 1
        records = [
            data for key, data in nested_data.items()
            if key[idx] == canton
        ]

        # Ensure all cantons have an entry for trips to themselves
